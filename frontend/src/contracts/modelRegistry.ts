...
export const ModelRegistryItemSchema = z.discriminatedUnion('id', [
  ...
  ReportedModelFieldsSchema.extend({
    id: z.literal('gru_step5'),
    family: z.literal('gru'),
  }),
  ReportedModelFieldsSchema.extend({
  ...
  ReportedModelFieldsSchema.extend({
    id: z.literal('rnn_step5'),
    family: z.literal('rnn'),
  }),
  ReportedModelFieldsSchema.extend({
...
export const ModelRegistryResponseSchema = z
  .strictObject({
    items: z.array(ModelRegistryItemSchema).length(5),
  })
  .refine((value) => new Set(value.items.map((item) => item.id)).size === 5, {
