...
describe('QUALITY panel family complete state', () => {
  it('renders all authoritative panels from independently selected section responses', () => {
    ...
    expect(screen.getByTestId('univariate-boxplot-Suhu')).not.toBeNull()
    expect(screen.getByTestId('univariate-boxplot-RH')).not.toBeNull()
    expect(screen.getByTestId('univariate-qq-Suhu')).not.toBeNull()
    expect(screen.getByTestId('univariate-qq-RH')).not.toBeNull()
    expect(screen.getByTestId('univariate-skew-Suhu')).not.toBeNull()
    expect(screen.getByTestId('univariate-skew-RH')).not.toBeNull()
    expect(screen.getAllByText(/aproksimasi berbasis histogram\/ECDF ter-bin/)).toHaveLength(2)
