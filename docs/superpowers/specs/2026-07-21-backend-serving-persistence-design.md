...
- FastAPI owns validation, response serialization, EDA queries computed on demand, migrations checks, seed orchestration, and database transactions.
...
| `inference_results` | `(device_id, window_end_ts, model_version)`; window start, score, threshold, anomaly flag, model/preprocessing/threshold hashes | Seeded artifacts only in M1. Windows satisfy `start < end`; every numeric output is finite. |
...
| `GET /api/inference-results` | required `device_id`, `from`, `to`; `bucket=raw`; `limit=500` (1–5,000; bucketed max 2,000); optional `model_version`, cursor | Seeded inference points only; `inference:<offset>` cursor. |
...
| `GET /api/model-evaluations/{version}` | catch-all path value already decoded once by Uvicorn/Starlette; use it directly as a nonempty version | One strict evaluation detail or 404; encoded slashes are valid version content. |
...
3. Tests cover `n1`…`n6`, shifted timestamps with preserved spacing, empty results, data gaps, half-open ranges, every default/bound, all four cursor prefixes, and malformed cursors returning 422.
